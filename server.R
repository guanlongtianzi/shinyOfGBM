...
shinyServer(function(input, output) {
  ...
  output$summary <- renderPrint({
    ...
    subdata <- train_data[which(train_data$yuqi != 'unknow'),]
    subpre <- train_data[which(train_data$yuqi == 'unknow'),]
    df <- data.table(subdata, keep.rownames = F)
    dfpre <- data.table(subpre, keep.rownames = F)
    dfall <- data.table(train_data, keep.rownames = F)
    sparse_matrix <<- sparse.model.matrix(yuqi~.-1, data = df)
    sparse_matrix_pre <<- sparse.model.matrix(yuqi~.-1, data = dfpre)
    sparse_matrix_all <<- sparse.model.matrix(yuqi~.-1, data = dfall)
    output_vector  <<-  df[,Y:=0][yuqi == "fraud",Y:=1][,Y]
    bst <<- xgboost(data = sparse_matrix, label = output_vector, max.depth = 4,eta = 1, nthread = 2, nround = 50,objective = "binary:logistic")
    summary(bst)
  })
  ...
  output$pr_curve <- renderPlot({
    ...
    n1 <- NCOL(train_data)
    pre <- predict(bst,sparse_matrix_all)
    or <- order(pre,decreasing = TRUE)

    ## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ## predictions,labels
    ## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    predictions <- pre[or]
    labels <- train_data[or,n1]
